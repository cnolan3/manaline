...
/// A client over any transport (WebSocket included).
pub fn spawn_transport(transport: crate::framing::BoxedTransport) -> (AsyncClient, mpsc::Receiver<ServerMessage>) {
    let (reader, writer) = crate::framing::MessageConnection::<ServerEnvelope, ClientEnvelope>::new(transport).split();
    start(reader, writer, Vec::new(), None)
}

/// The one way in: a single supervisor task owns the connection, so handing it
/// a new one is all a reconnect has to be.
fn start(
    reader: MessageReader<ServerEnvelope>,
    writer: MessageWriter<ClientEnvelope>,
...
fn link(mut reader: MessageReader<ServerEnvelope>, writer: MessageWriter<ClientEnvelope>) -> Link {
