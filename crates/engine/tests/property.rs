...
fn config(db: &Arc<engine::CardDb>, seats: usize) -> GameConfig {
    let format = if seats == 2 { Format::cube() } else { Format::builtin("free-for-all").unwrap() };
    let players = (0..seats)
        .map(|i| {
            let (_, text) = cards::DECKS[i % cards::DECKS.len()];
            PlayerSetup { name: format!("Bot{i}"), deck: cards::parse_decklist(text, db).unwrap() }
        })
        .collect();
    GameConfig { format, players, cards: db.clone(), starting_player: None }
}
...
fn check_invariants(game: &Game, seats: usize) {
    ...
    if game.is_over().is_some() {
        ...
        assert!(!must.is_empty(), "must_act is empty while the game is not over (turn {}, {:?})", game.turn, game.phase);
        for s in 0..seats {
...
fn run_game(db: &Arc<engine::CardDb>, seats: usize, seed: u64) -> Game {
    ...
    let mut life = LifeTracker { life: vec![game.format.starting_life; seats] };
    let mut turn_seen = 0;
    check_invariants(&game, seats);
    let mut applied = 0;
    while game.is_over().is_none() {
        assert!(applied < MAX_ACTIONS, "game did not finish within {MAX_ACTIONS} actions (turn {})", game.turn);
        let must = game.must_act();
...
fn seeds() -> Vec<u64> {
    let n: u64 = std::env::var("MANALINE_PROPERTY_SEEDS").ok().and_then(|s| s.parse().ok()).unwrap_or(6);
    (1..=n).collect()
}
