...
use engine::{
    ActReason, Action, AttackTarget, DamageTarget, Game, Keyword, ObjectId, PendingChoice, Phase, RulesError, Seat,
    Target, Zone, EQUIP_ABILITY,
};
use std::sync::Arc;
...
fn hand_card(game: &Game, seat: Seat, name: &str) -> ObjectId {
    game.players[seat.index()].hand.iter().copied().find(|&id| game.object_name(id) == name).unwrap_or_else(|| panic!("{name} not in hand"))
}

fn bf(game: &Game, seat: Seat, name: &str) -> ObjectId {
    game.players[seat.index()].battlefield.iter().copied().find(|&id| game.object_name(id) == name).unwrap_or_else(|| panic!("{name} not on battlefield"))
}

fn bfs(game: &Game, seat: Seat, name: &str) -> Vec<ObjectId> {
    game.players[seat.index()].battlefield.iter().copied().filter(|&id| game.object_name(id) == name).collect()
}
...
fn block_with(game: &mut Game, seat: Seat, blocks: &[(ObjectId, ObjectId)]) {
    assert!(matches!(game.pending, Some(PendingChoice::DeclareBlockers { .. })), "{:?}", game.pending);
...
fn lightning_strike_targets_any_target_and_kills() {
    ...
    let casts: Vec<Action> = game.legal_actions(Seat(0)).into_iter().filter(|a| matches!(a, Action::CastSpell { .. })).collect();
    let targets: Vec<Vec<Target>> = casts.iter().map(|a| match a { Action::CastSpell { targets, .. } => targets.clone(), _ => unreachable!() }).collect();
...
fn sorceries_only_at_sorcery_speed() {
    let mut game = TestGame::new(db(), 2).battlefield(Seat(0), "Mountain").hand(Seat(0), "Flame Slash").battlefield(Seat(1), "Grizzly Bears").build();
    assert!(game.legal_actions(Seat(0)).iter().any(|a| matches!(a, Action::CastSpell { .. })));
    pass_both(&mut game);
    assert_eq!(game.phase, Phase::BeginCombat);
    assert!(!game.legal_actions(Seat(0)).iter().any(|a| matches!(a, Action::CastSpell { .. })), "not in combat");
}
...
fn counterspell_counters_a_creature_spell() {
    ...
    assert!(!game.legal_actions(Seat(1)).iter().any(|a| matches!(a, Action::CastSpell { object, .. } if *object == negate)));
...
fn dark_ritual_mana_can_pay_for_a_spell_in_the_same_phase() {
    ...
    assert!(!game.legal_actions(Seat(0)).iter().any(|a| matches!(a, Action::CastSpell { object, .. } if *object == murder)));
    cast(&mut game, Seat(0), "Dark Ritual", &[]);
    resolve_top(&mut game);
    assert_eq!(game.players[0].mana_pool.total(), 3);
    let cast_murder = game.legal_actions(Seat(0)).into_iter().find(|a| matches!(a, Action::CastSpell { object, .. } if *object == murder)).expect("Murder castable from the pool");
...
fn cruel_edict_asks_the_opponent_which_creature_to_sacrifice() {
    ...
    resolve_top(&mut game);
    assert!(matches!(game.pending, Some(PendingChoice::Sacrifice { seat: Seat(1), .. })), "{:?}", game.pending);
    ...
    game.apply(Seat(1), &Action::ChooseTargets { targets: vec![Target::Object(giant)] }).unwrap();
...
fn mind_rot_lets_the_target_choose_what_to_discard() {
    ...
    resolve_top(&mut game);
    assert!(matches!(game.pending, Some(PendingChoice::EffectDiscard { seat: Seat(1), count: 2, .. })));
...
fn elvish_visionary_draws_on_entering() {
    let mut game = TestGame::new(db(), 2).battlefield(Seat(0), "Forest").battlefield(Seat(0), "Forest").hand(Seat(0), "Elvish Visionary").build();
...
fn man_o_war_asks_for_a_target_then_bounces() {
    ...
    resolve_top(&mut game);
    assert!(matches!(game.pending, Some(PendingChoice::ChooseTargets { seat: Seat(0), .. })), "{:?}", game.pending);
    assert_eq!(game.must_act().get(&Seat(0)), Some(&ActReason::Choice));
    let choices = game.legal_actions(Seat(0));
    assert_eq!(choices.iter().filter(|a| matches!(a, Action::ChooseTargets { .. })).count(), 2, "bears or itself");
    game.apply(Seat(0), &Action::ChooseTargets { targets: vec![Target::Object(bears)] }).unwrap();
    assert_eq!(game.stack.len(), 1);
    resolve_top(&mut game);
    assert_eq!(game.objects[bears].zone, Zone::Hand);
}
...
fn festering_goblin_dies_trigger_shrinks_a_creature() {
    ...
    assert!(matches!(game.pending, Some(PendingChoice::ChooseTargets { seat: Seat(0), .. })), "{:?}", game.pending);
    game.apply(Seat(0), &Action::ChooseTargets { targets: vec![Target::Object(hawk)] }).unwrap();
    resolve_top(&mut game);
...
fn prowess_pumps_on_a_noncreature_spell() {
    let mut game = TestGame::new(db(), 2).battlefield(Seat(0), "Bloodfire Expert").battlefield(Seat(0), "Mountain").hand(Seat(0), "Shock").build();
...
fn prodigal_pyromancer_pings_and_respects_summoning_sickness() {
    ...
    let acts: Vec<Action> = game.legal_actions(Seat(0)).into_iter().filter(|a| matches!(a, Action::ActivateAbility { object, .. } if *object == pinger)).collect();
    assert_eq!(acts.len(), 3, "bears? no: itself, and both players: {acts:?}");
    let ping = acts.iter().find(|a| matches!(a, Action::ActivateAbility { targets, .. } if targets == &vec![Target::Player(Seat(1))])).unwrap().clone();
    game.apply(Seat(0), &ping).unwrap();
    assert!(game.objects[pinger].tapped);
    resolve_top(&mut game);
    assert_eq!(game.players[1].life, 19);

    let mut sick = TestGame::new(db(), 2).battlefield(Seat(0), "Mountain").battlefield(Seat(0), "Mountain").battlefield(Seat(0), "Mountain").hand(Seat(0), "Prodigal Pyromancer").build();
    cast(&mut sick, Seat(0), "Prodigal Pyromancer", &[]);
    resolve_top(&mut sick);
    assert!(!sick.legal_actions(Seat(0)).iter().any(|a| matches!(a, Action::ActivateAbility { .. })), "summoning sick: no {{T}}");
}
...
fn elves_make_mana_and_the_archdruid_scales_with_elves() {
    ...
    assert_eq!(sources.iter().find(|(id, _, _)| *id == druid).map(|(_, _, n)| *n), Some(3), "three Elves");
    let wurm = hand_card(&game, Seat(0), "Craw Wurm");
    assert!(game.legal_actions(Seat(0)).iter().any(|a| matches!(a, Action::CastSpell { object, .. } if *object == wurm)), "{{4}}{{G}}{{G}} from 3 Forests + Elves");
...
fn sacrifice_costs_enumerate_each_candidate() {
    let mut game = TestGame::new(db_with_extras(), 2).battlefield(Seat(0), "Test Sac Outlet").battlefield(Seat(0), "Grizzly Bears").build();
    let bears = bf(&game, Seat(0), "Grizzly Bears");
    let acts: Vec<Action> = game.legal_actions(Seat(0)).into_iter().filter(|a| matches!(a, Action::ActivateAbility { .. })).collect();
    assert_eq!(acts.len(), 2, "sacrifice itself or the bears: {acts:?}");
    let sac_bears = acts.iter().find(|a| matches!(a, Action::ActivateAbility { payment, .. } if payment.sacrifice == vec![bears])).unwrap().clone();
...
fn counters_change_stats_and_annihilate() {
    let mut game = TestGame::new(db_with_extras(), 2).battlefield(Seat(0), "Forest").battlefield(Seat(0), "Grizzly Bears").hand(Seat(0), "Test Counters").build();
    let bears = bf(&game, Seat(0), "Grizzly Bears");
...
fn flying_needs_flying_or_reach_to_block() {
    ...
    let err = game.apply(Seat(1), &Action::DeclareBlockers { blocks: vec![(bears, drake)] }).unwrap_err();
    assert!(matches!(err, RulesError::IllegalAction { .. }));
    assert!(!game.legal_actions(Seat(1)).contains(&Action::DeclareBlockers { blocks: vec![(bears, drake)] }));
...
fn first_strike_kills_before_taking_damage() {
    let mut game = TestGame::new(db(), 2).battlefield(Seat(0), "Youthful Knight").battlefield(Seat(1), "Grizzly Bears").build();
...
fn trample_assigns_the_rest_to_the_player() {
    let mut game = TestGame::new(db(), 2).battlefield(Seat(0), "Colossal Dreadmaw").battlefield(Seat(1), "Grizzly Bears").build();
    let maw = bf(&game, Seat(0), "Colossal Dreadmaw");
    let bears = bf(&game, Seat(1), "Grizzly Bears");
    attack_with(&mut game, Seat(0), &[maw], Seat(1));
    block_with(&mut game, Seat(1), &[(bears, maw)]);
    assert!(matches!(game.pending, Some(PendingChoice::AssignDamage { seat: Seat(0), .. })), "trample with a blocker is a choice");
    ...
    assert!(suggestions.iter().any(|a| a.canonical() == lethal_then_player.canonical()), "{suggestions:?}");
    let greedy = Action::AssignCombatDamage { attacker: maw, assignments: vec![(DamageTarget::Object(bears), 1), (DamageTarget::Player(Seat(1)), 5)] };
    assert!(matches!(game.apply(Seat(0), &greedy), Err(RulesError::IllegalAction { .. })), "lethal first");
...
fn vigilance_haste_menace_and_flash() {
    ...
    let bears = bf(&game, Seat(1), "Grizzly Bears");
    let giant = bf(&game, Seat(1), "Hill Giant");
    let one_block = Action::DeclareBlockers { blocks: vec![(bears, menace)] };
    assert!(matches!(game.apply(Seat(1), &one_block), Err(RulesError::IllegalAction { .. })), "menace");
    block_with(&mut game, Seat(1), &[(bears, menace), (giant, menace)]);
    assert!(matches!(game.pending, Some(PendingChoice::AssignDamage { .. })));
    let split = game.legal_actions(Seat(0)).into_iter().find(|a| matches!(a, Action::AssignCombatDamage { .. })).unwrap();
    ...
    assert!(game.legal_actions(Seat(1)).iter().any(|a| matches!(a, Action::CastSpell { object, .. } if *object == flash)), "flash");
}
...
fn hexproof_blocks_opponents_targets_but_not_your_own() {
    ...
    let strikes: Vec<Action> = game.legal_actions(Seat(0)).into_iter().filter(|a| matches!(a, Action::CastSpell { .. })).collect();
    assert!(!strikes.iter().any(|a| matches!(a, Action::CastSpell { targets, .. } if targets.contains(&Target::Object(bear)))));
    assert_eq!(strikes.len(), 2, "only the two players");
    game.apply(Seat(0), &Action::PassPriority).unwrap();
    assert!(game.legal_actions(Seat(1)).iter().any(|a| matches!(a, Action::CastSpell { targets, .. } if targets.contains(&Target::Object(bear)))), "its controller can target it");
}
...
fn views_describe_the_stack_and_abilities() {
    let mut game = TestGame::new(db(), 2).battlefield(Seat(0), "Prodigal Pyromancer").battlefield(Seat(0), "Furnace Whelp").battlefield(Seat(0), "Mountain").build();
    let view = game.view(Seat(0));
    let pinger = bf(&game, Seat(0), "Prodigal Pyromancer");
    assert_eq!(view.objects[&pinger].abilities, vec!["{T}: ~ deals 1 damage to any target.".to_string()]);
    let ping = game.legal_actions(Seat(0)).into_iter().find(|a| matches!(a, Action::ActivateAbility { object, targets, .. } if *object == pinger && targets == &vec![Target::Player(Seat(1))])).unwrap();
...
fn big_mana_producers_are_offered_and_custom_payments_are_accepted() {
    ...
    assert!(payments.iter().any(|p| p.tap.len() == 4 && p.tap.contains(&druid) && forests.iter().all(|f| p.tap.contains(f))), "{payments:?}");
    ...
    let custom = Action::CastSpell {
        object: overrun,
        targets: vec![],
        payment: engine::ManaPayment { tap: vec![druid, forests[2]], ..Default::default() },
    };
    ...
    assert!(game.legal_actions(Seat(0)).iter().any(|a| matches!(a, Action::CastSpell { object, .. } if *object == growth)), "Forests left up");

    // One that does not cover the cost is refused.
    let short = Action::CastSpell {
        object: growth,
        targets: vec![Target::Object(druid)],
        payment: engine::ManaPayment { tap: vec![druid], ..Default::default() },
    };
    assert!(matches!(game.apply(Seat(0), &short), Err(RulesError::IllegalAction { .. })), "tapped source");
}
