...
- The AI learns it has priority two ways: a blocking `wait_for_priority` tool, and a printed nudge in the human's TUI as fallback.  
...
- **Unix domain socket** at `$XDG_RUNTIME_DIR/manaline/<game-id>.sock` — default for local clients. Ships in the initial build.  
...
| `hello { token }` | `welcome { seat | spectator, game_id, format, state }` |
...
1. Spawns `manaline-daemon` as a child process on a fresh Unix socket (a random path under `$XDG_RUNTIME_DIR/manaline/`; no `--tcp` listener unless asked), with a `--parent-pid` so the daemon exits if `play` dies.  
...
impl Game {
    ...
    pub fn legal\_actions(\&self, seat: Seat) \-\> Vec\<Action\>;
    ...
    pub fn view\_spectator(\&self) \-\> GameView;            // all hidden info removed

    pub fn is\_over(\&self) \-\> Option\<Outcome\>;            // Outcome::Winner(Seat) | Draw
...
pub struct Game {
    ...
    pub turn\_order: Vec\<Seat\>,        // seats still in the game, in APNAP order from the starting player

    pub active\_player: Seat,
    ...
    pub passed\_in\_succession: u8,     // \== turn\_order.len() with empty stack → advance phase
    ...
    pub pending: Option\<PendingChoice\>,          // targets, blockers, mana payment, etc.
...
pub struct PlayerState {
    ...
    pub life: i32,                    // starts at format.starting\_life
    ...
    pub commander\_damage: HashMap\<ObjectId, i32\>,  // damage taken from each commander (rule 903.10a)

    pub commander\_casts: HashMap\<ObjectId, u8\>,    // for commander tax

    pub mana\_pool: ManaPool,

    pub lands\_played\_this\_turn: u8,
...
pub struct GameObject {
    ...
    pub summoning\_sick: bool,

    pub damage: i32,

    pub counters: Counters,           // \+1/+1, \-1/-1

    pub attached\_to: Option\<ObjectId\>,
...
pub enum Action {
    ...
    OrderBlockers { attacker: ObjectId, order: Vec\<ObjectId\> },
...
Combat is the hairiest part. Sequence: `BeginCombat` → `DeclareAttackers` (active player acts, then priority) → `DeclareBlockers` (defending player acts, then attacker orders blockers, then priority) → `CombatDamage` (first strike round if any, then regular) → `EndCombat`. Skip `DeclareBlockers`/`CombatDamage` if no attackers.
...
- A property test: play N random games choosing uniformly from `legal_actions`; assert invariants after every action (life totals consistent with damage events, no object in two zones, `legal_actions` never empty while game is not over, stack empties within bounded passes).
...
Cards are **data, not code**. Each card is a file in `crates/cards/data/<set>/<name>.ron` holding a small AST — the *card IR* — that the engine interprets. The engine implements a fixed vocabulary of primitives (effects, costs, targets, triggers, statics); a card is a composition of those primitives and nothing else.
...
// crates/cards/data/core/lightning\_strike.ron
...
// crates/cards/data/core/elvish\_archdruid.ron
...
pub enum Effect {
    ...
    Conditional { if\_: Condition, then: Box\<Effect\>, else\_: Option\<Box\<Effect\>\> },
...
Format(
    ...
    mulligan: London(free\_first: true),      // house-rule toggle

    deck: Deck(size: Exact(100), singleton: true, includes\_commander: true),
...
Format(
    ...
    mulligan: London(free\_first: false),

    deck: Deck(size: Range(40, 60), singleton: false, includes\_commander: false),

    rules: \[\],

    legality: Legality(pool: Explicit("crates/cards/data/core"), banned: \[\], allowed: \[\]),
...
pub struct Format {
    ...
    pub starting\_life: i32,

    pub starting\_hand: u8,
...
pub enum CardPool {
    ...
    Explicit(PathBuf),                 // a directory of IR files \= the allowlist
...
Format(
    ...
    deck: Deck(size: Min(40), singleton: false, includes\_commander: false),
    ...
    limited: Some(Limited(
        ...
        build\_time: Some(1800),          // seconds before the lobby auto-readies you
...
- Validate `act` messages: correct seat, `state_version` current, action present in `legal_actions`, then `Game::apply`.  
...
- Track *who has priority* and expose it so clients can block on it (see §7).  
...
Concurrency is intentionally boring: a single `tokio::sync::Mutex<Game>` and a `tokio::sync::watch` channel carrying `(turn, phase, priority_holder, state_version)`. `wait_for_priority` is literally `watch::Receiver::wait_for(|s| s.priority_holder == my_seat)`. This design already handles N seats; it's per-game, not per-player, and the daemon never needs to know how many clients are waiting.
...
The AI-priority nudge lives in the header line: when another seat holds priority, the header reads `Waiting on Claude (seat 1)…` and, if no action arrives within a configurable timeout (default 30 s), the footer switches to `Seat 1 has priority — nudge your agent, or [Enter] to re-notify`. Pressing Enter re-fires the `priority` watch event and appends a chat event `"[system] It's your priority, Claude."` that the agent sees on its next `get_state`.
...
| `get_legal_actions` | — | Numbered list of legal actions with human-readable descriptions; empty list if not your priority |
| `take_action` | `{ action_id: u32 }` or `{ action: Action }` | Result: events that occurred, new state, and — if you still have priority — the next legal actions |
| `wait_for_priority` | `{ timeout_seconds?: u32 }` (default 300\) | Blocks until this seat has priority or the game ends; returns state \+ legal actions. Returns `{ timed_out: true }` on timeout so the agent can loop |
...
- Prompt `play-a-game` — the recommended agent loop, as an MCP prompt so any client can pull it: *read the primer; loop { wait\_for\_priority; think; take\_action until you pass; }; use `say` to talk to your opponent.*
...
3. Agent calls `wait_for_priority`. Mulligan decision arrives first.  
4. Game proceeds. Whenever it's the agent's priority, the blocked `wait_for_priority` returns with state and legal actions. The agent acts, possibly several times (cast, then pass), then calls `wait_for_priority` again.  
5. If the agent's client kills long tool calls (some do at 60 s), `wait_for_priority` returns `{ timed_out: true }` and the agent just calls it again. Meanwhile the human sees `Waiting on Claude…` in the TUI and can press Enter to post a system chat nudge, which the agent sees the next time it looks.  
...
**M0 — Engine skeleton (1–2 weeks)** `Format` struct and `cube.ron`. N-player state (`Vec<PlayerState>`, `turn_order`, elimination). Turn/phase state machine, priority passing, lands, vanilla creatures, combat with no keywords, state-based actions (lethal damage, 0 life). `--vs random` opponent that picks uniformly from `legal_actions`. Property test in place **and run at N=2, 3, and 4 seats** — three random bots in a pod is the cheapest multiplayer test you'll ever write. *Runnable: random bots finish a game of Forests and Grizzly Bears without panicking, at any seat count.*

**M1 — Daemon \+ TUI (1–2 weeks)** Protocol crate, daemon with lobby, seat tokens, `--socket` and `--tcp` listeners, replay log. TUI rendering battlefield/hand/stack/log with keyboard actions generated from `legal_actions`; opponent region built as a list of panes. *Runnable: a human beats the random bot in the terminal. Also runnable, quietly: two TUIs on one machine via `--tcp 127.0.0.1`, which is networked play in everything but the WAN.*

**M2 — MCP (1 week)** `rmcp` server with all tools in §7.1, both transports, the rules primer resource, the `play-a-game` prompt, the Enter-to-nudge fallback. *Runnable: a human plays Claude Code. This is the demo.*

**M3 — IR, spells, abilities, keywords (2–3 weeks)** `cardir` types, validator, and renderer. `engine::interp`. Stack with instants/sorceries, targeting, mana payment solver, triggered abilities, evergreen keywords, "until end of turn" modifiers, auras and equipment. First \~60 cards as `.ron` with round-trip tests. The IR vocabulary settles here — expect to reshape the enums a few times before it does, and don't start M5 until it has.

**M4 — The cube and deck files (2–3 weeks, parallelisable)** Remaining \~140 cards, one test each. `carddb` with the Scryfall bulk cache and `cards update`. `deckstats` parser for the text deck format, `deck check` with per-card reasons, `deck new`. Six starter decks. `ingest roundtrip` in CI.

**M5 — Ingestion tool, few-shot backend (1–2 weeks)** `ingest` crate: API backend, embedding-selected few-shot examples, validate/retry loop, round-trip diff, report. `ingest eval` scored against the held-out cube cards. *Runnable: point it at a real set and get a report with a supported/unsupported split.* First real use: whichever set you most want to play with.

**M6 — Polish and the deckbuilder** Mulligans, 80×24 layout, colour themes, replay stepping in the TUI, `cards show` with ASCII card render, README with a screencast. `cardsearch` query language and index; `deck stats`; the three-pane `deck edit` TUI reading and writing the same file, with live legality and sample hands drawn by the engine. `search_cards` / `deck_stats` on the MCP server.

**M7 — Fine-tuned ingestion backend (optional, 2–4 weeks)** Forge-script → IR converter for supported primitives, dataset assembly and augmentation, LoRA fine-tune, `--backend local`, eval against the M5 baseline on schema-valid / round-trip / test-pass rates.
...
- **Mana payment enumeration blow-up.** With many lands of overlapping colours, "one `CastSpell` per distinct payment" gets long. Plan: enumerate by *colour combination tapped*, not by specific permanents, and let the daemon pick concrete permanents (preferring lands over mana creatures, basic over non-basic). Revisit if agents seem confused.  
- **Timeout defaults for `wait_for_priority`.** 300 s is a guess. Instrument what clients actually tolerate.  
...
- **Multiplayer priority and agents.** In a four-seat pod, three of the seats may be agents waiting on `wait_for_priority`. That's fine for the daemon, but the human's "nudge" key needs to nudge the seat that actually holds priority, and the header needs to say which seat that is. Small, but easy to get wrong.  
