...
- **The layout is a list of opponent panes, not "the opponent pane."** The initial build renders one; with N−1 opponents the top region becomes a row of collapsed panes (name, life, hand/library counts, creature count, commander damage you've taken from them) and `Tab` expands one at a time. Attack declaration and "target opponent" prompts pick a seat from that row. Building the two-player layout as the N=2 case of this costs an afternoon now and saves a rewrite later. A spectator has no seat, so the near region draws a seat they choose instead — seat 0 by default, `Shift-Tab` moves it, and with two seats the sides are fixed so both players are on screen at once.
...
| `wait_for_turn`      | `{ timeout_seconds?: u32, auto_pass?: bool }` (defaults 45, true) | Blocks until this seat has a **real decision** or the game ends; returns state + legal actions (headed `LEGAL ACTIONS (state_version N)`), any table chat not yet shown, and says *why* — the engine's `ActReason` for this seat, verbatim: `"priority" \| "declare_attackers" \| "declare_blockers" \| "assign_damage" \| "mulligan" \| "bottom_cards" \| "discard" \| "choice"`. With `auto_pass` (the default) quiet priority windows are passed for the agent: it is woken for any non-priority reason, any window with something on the stack, its own main phases and declare attackers, and an opponent's declare attackers (once attackers are in), declare blockers and end step; every other window with an empty stack is passed, whether or not it holds an instant, and `auto_passed` counts those. Returns `{ timed_out: true }` on timeout so the agent can loop |
...
```
manaline play      --deck <file> [--vs mcp|human|random] [--seats <spec>] [--opp-deck <file>]
                   [--seat-deck N=<deck>] [--format <name>] [--seed <u64>] [--server <endpoint>]
                   [--tcp <addr>] [--watch] [--name <n>] [--theme <t>]
                   [--idle-warn <s>] [--idle-concede <s>] [--abandon-after <s>]
...
manaline join      <code> --server <endpoint> --deck <file> [--token <t>] [--name <n>]  # tier 1/2: via a lobby server
manaline create    --server <endpoint> [--format <name>] [--seats <n>] [--seed <u64>]   # tier 1/2: prints the game code
...
`create`, `join <code>` and `play --server` are the tier-1 client side. `--server` takes any endpoint form — `host:port`, `ws://…`, `wss://…` — and a `join` argument is a game code rather than an endpoint exactly when it is six characters of the code alphabet, which has no `:`, `/` or `.` in it, so the two can never be confused; a code with no `--server` and an endpoint with no `--token` are both refused with the flag they want. `create` connects, sends `create_game`, and prints the code, one `manaline join <code> --server <endpoint> --deck <deck>` line per seat, the raw seat tokens for a client that wants one, and the spectator token; the connection has no standing at the table it just made. `join <code>` sends `join_game`, takes the token it gets back, and runs the ordinary client against the server — `--token` with a code skips that step. `play --server` creates the game on the server instead of spawning a daemon and is otherwise identical: it publishes the same `GameMarker` (whose one address is now the server's, so agent seats work unchanged), runs the bots and the TUI against the server, prints join-by-code lines for `human` seats, and on exit withdraws only the marker — the server's `--abandon-after` clears the game up. It takes none of the flags that configure a daemon of our own (`--tcp`, `--idle-*`, `--abandon-after`), because the server sets its own policy.
